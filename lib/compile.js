...
var Compiler = (function () {
    ...
    Compiler.parsePoItems = function (items, replaceBrowserConvert) {
        replaceBrowserConvert = replaceBrowserConvert !== false;
        var strings = {};
    ...
    Compiler.prototype.convertPo = function (inputs) {
        ...
        inputs.forEach(function (input) {
            ...
            localeList.push({
                locale: catalog.headers.Language,
                strings: Compiler.parsePoItems(catalog.items)
            });
        });

        return this.format(localeList);
    };

    Compiler.prototype.format = function (localeList) {
        var format = formats[this.options.format];
        var locales = [];
        for (var i = 0; i < localeList.length; i++) {
            var localeMap = localeList[i];
            locales.push(format.addLocale(localeMap.locale, localeMap.strings));
        }
